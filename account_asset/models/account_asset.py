...
class AccountAsset(models.Model):
    ...
    account_depreciation_id = fields.Many2one('account.account', string='Depreciation Account', states={'close': [('readonly', True)]}, domain="[('internal_type', '=', 'other'), ('deprecated', '=', False), ('company_id', '=', company_id), ('is_off_balance', '=', False)]", help="Account used in the depreciation entries, to decrease the asset value.")
    account_depreciation_expense_id = fields.Many2one('account.account', string='Expense Account', states={'close': [('readonly', True)]}, domain="[('internal_type', '=', 'other'), ('deprecated', '=', False), ('company_id', '=', company_id), ('is_off_balance', '=', False)]", help="Account used in the periodical entries, to record a part of the asset as expense.")
    ...
    def _compute_acquisition_date(self):
        for record in self:
            record.acquisition_date = record.acquisition_date or min(record.original_move_line_ids.mapped('date') + [record.prorata and record.prorata_date or record.first_depreciation_date or fields.Date.today()])
    ...
    @api.onchange('original_value', 'salvage_value', 'acquisition_date', 'method', 'method_progress_factor', 'method_period',
                  'first_depreciation_date', 'prorata', 'method_number', 'already_depreciated_amount_import', 'prorata_date')
    ...
    def compute_depreciation_board(self):
        self.ensure_one()
        # If prorata is a start of period, we set the prorata field to False
        # to prevent a wrong depreciation rate in the report.
        # Because the number of moves cannot be predictive (+/- 1).
        # Indeed the last move can be zero, and it is not created.
        if self.prorata and self.prorata_date:
            fiscal_year_start = self.company_id.compute_fiscalyear_dates(self.prorata_date)['date_from']
            if (self.method_period == '1' and self.prorata_date.day == 1) \
                    or (self.method_period == '12' and self.prorata_date == fiscal_year_start):
                self.prorata = False
    ...
    def _get_disposal_moves(self, invoice_line_ids, disposal_date):
        ...
        for asset, invoice_line_id in zip(self, invoice_line_ids):
            ...
            invoice_amount = copysign(invoice_line_id.balance, -initial_amount)

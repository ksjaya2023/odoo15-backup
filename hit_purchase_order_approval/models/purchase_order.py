...
class PurchaseOrder(models.Model):
    ...
    user_eligibility_print = fields.Boolean(
        string='User eligibility to print', default=False, compute='_check_state')

    @api.depends('state', 'purchasing_group_rel', 'approval_sec')
    def _check_state(self):
        for record in self:
            record.user_eligibility_print = False
            if record.state == 'done':
                if (record.purchasing_group_rel == 'Buyer HO Asset' or \
                    record.purchasing_group_rel == 'Buyer HO Non Asset') and record.approval_sec == 4:
                    record.user_eligibility_print = True
                elif (record.purchasing_group_rel != 'Buyer HO Asset' and \
                    record.purchasing_group_rel != 'Buyer HO Non Asset') and record.approval_sec == 3:
                    record.user_eligibility_print = True
    ...
    def _check_whos_responsible(self):
        for record in self:
            ...
            if (record.purchasing_group_rel == 'Buyer HO Non Asset' or \
                record.purchasing_group_rel == 'Buyer HO Asset' and seq > 3) or\
                    (record.purchasing_group_rel != 'Buyer HO Non Asset' and \
                     record.purchasing_group_rel != 'Buyer HO Asset' and seq > 2):
    ...
    def action_release(self):
        if (self.state == 'to approve' or self.state == 'purchase' and self.approval_sec < 4):
            ...
            if checked:
                if (self.purchasing_group_rel == 'Buyer HO Non Asset' or \
                    self.purchasing_group_rel == 'Buyer HO Asset' and seq == 3) or\
                        (self.purchasing_group_rel != 'Buyer HO Non Asset' and \
                        self.purchasing_group_rel != 'Buyer HO Asset' and seq == 2):
                    self.button_confirm()
                if seq == 0:

...
class AEATAccountFinancialReport(models.Model):
    ...
    def _boe_export_mod303(self, options, current_company, period, year):
        ...
        if options['date']['date_from'] >= '2023-01-01':
            rslt += self._generate_mod_303_page_did(options, current_company, period, year, casilla_lines_map)
    ...
    def _generate_mod_303_page1(self, options, current_company, period, year, casilla_lines_map):
        ...
        # Casillas
        if options['date']['date_from'] >= '2023-01-01':
            rslt += self._boe_format_number(casilla_lines_map.get('150', 0), length=17, decimal_places=2, in_currency=True)
            rslt += self._boe_format_number(0, length=5)  # Casilla 151 is constant
            rslt += self._boe_format_number(casilla_lines_map.get('152', 0), length=17, decimal_places=2, in_currency=True)
        ...
        if options['date']['date_from'] >= '2023-01-01':
            rslt += self._boe_format_number(casilla_lines_map.get('153', 0), length=17, decimal_places=2, in_currency=True)
            rslt += self._boe_format_number(500, length=5)  # Casilla 154 is constant
            rslt += self._boe_format_number(casilla_lines_map.get('155', 0), length=17, decimal_places=2, in_currency=True)
        ...
        if options['date']['date_from'] >= '2023-01-01':
            rslt += self._boe_format_number(casilla_lines_map.get('156', 0), length=17, decimal_places=2, in_currency=True)
            rslt += self._boe_format_number(175, length=5)  # Casilla 157 is constant
            rslt += self._boe_format_number(casilla_lines_map.get('158', 0), length=17, decimal_places=2, in_currency=True)

        rslt += self._boe_format_number(casilla_lines_map['16'], length=17, decimal_places=2, in_currency=True)
        rslt += self._boe_format_number(50, length=5) # Casilla 17 is constant (any of 00000, 00050, 00062)
    ...
    def _generate_mod_303_page3(self, options, current_company, period, year, casilla_lines_map):
        ...
        for casilla in (62, 63, 74, 75):
            rslt += self._boe_format_number(casilla_lines_map[str(casilla)], length=17, decimal_places=2, signed=True, in_currency=True)
        rslt += self._boe_format_number(0, length=17)  # Normally casilla 76 (Regularization of quotas art. 80.Cinco.5ª LIVA)
        ...
        if options['date']['date_from'] >= '2023-01-01':
            rslt += self._boe_format_number(casilla_lines_map['70'], length=17, decimal_places=2, in_currency=True)  # Unsigned from 2023 on
        else:
            rslt += self._boe_format_number(casilla_lines_map['70'], length=17, decimal_places=2, signed=True, in_currency=True)

        if options['date']['date_from'] >= '2023-01-01':
            rslt += self._boe_format_number(casilla_lines_map.get('109', 0.0), length=17, decimal_places=2, in_currency=True)
    ...
    def _generate_mod_303_page_did(self, options, current_company, period, year, casilla_lines_map):
        rslt = self._boe_format_string('<T303DID00>')
        ...
        rslt += self._boe_format_string(bic if boe_wizard.declaration_type == 'X' else '', length=11)
        rslt += self._boe_format_string(iban, length=34)
        ...
        if boe_wizard.declaration_type == 'X':
            ...
            rslt += self._boe_format_string('', length=137)
        ...
        # Reserved by AEAT
        rslt += self._boe_format_string(' ' * 617)

...
class SaleSubscription(models.Model):
    ...
    name = fields.Char(required=True, tracking=True, default="New", copy=False)
    ...
    def partial_invoice_line(self, sale_order, option_line, refund=False, date_from=False):
        ...
        ratio, message, period_msg = self.with_context(lang=sale_order.partner_id.lang)\
            ._partial_recurring_invoice_ratio(date_from=date_from)
        if message != "":
    ...
    def action_subscription_invoice(self):
        ...
        action["context"] = {
            "create": False,
            "default_invoice_origin": self.code,
    ...
    def _recurring_create_invoice(self, automatic=False, batch_size=20):
        ...
        if len(self) > 0:
            ...
            subscriptions = self.search(self._get_subscription_domain_for_invoicing(current_date, subscription_payment_exception_tag | batch_tag), limit=batch_size + 1)
...
class SaleSubscriptionLine(models.Model):
    ...
    def _compute_amount(self):
        ...
        for line in self:
            ...
            fpos = fpos_obj.with_company(company.id).get_fiscal_position(partner.id)
            tax = line.product_id.sudo().taxes_id.filtered_domain([('company_id', '=', company.id)])
            if fpos:
            ...
            line.price_subtotal = price * (100.0 - line.discount) / 100.0
    ...
    def onchange_product_quantity(self):
        if not self.product_id:
            ...
            if pricelist.discount_policy == 'without_discount' and pricelist_price:
                discount = max(0, (base_price - pricelist_price) * 100 / base_price) if base_price != 0 else 0
            ...
            price_unit = product._get_tax_included_unit_price(
                ...
                product_currency=pricelist and pricelist.currency_id or product.currency_id, # if pricelist.currency_id == product.currency id; it is the same, otherwise we will use pricelist anyways
            )
            self.update({
